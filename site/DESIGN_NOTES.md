...
- 视图模型 `{x: 左缘年份, ppy: px/年, y}`；拖拽直控（1:1，不骗手），释放转惯性（`exp(-4.5t)` 衰减），聚焦/快跳走 tween（easeInOutCubic）。
- **滚轮 = 斜向漫游（非欧几里得）**：无修饰滚轮 → 时间速度冲量 `navV`（`exp(-2.8t)` 衰减，上限 1.5 屏年/s），loop 的 `flow` 分支匀速前进 + y 追贴「存活带」——`rowForTime` 二分 t−40 岁 cohort（±60 行内就近落回存活者；空旷段 cohort 线走平 = 纯水平快进时间）。**⌘/ctrl+滚轮 = 缩放**（触控板捏合天然带 ctrlKey，地图惯例）；聚焦/拖拽/tween 中滚轮不漫游；RM 直映无惯性。滚轮向下 = 时间前进，触控板横扫手感待真机调。
- **Poincaré 简化时间压缩（2026-09 定稿）**：分段线性 年↔warp，`W_CORE=[-800,250]` 斜率 1（春秋战国→秦汉三国正常比例），两侧 `W_EDGE=1/3`（时间 3 倍密集）；锚点是常量可调。视图 `{x, ppy}` 全程以 warp 年为单位——拖拽/缩放/漫游速度换算零改动，漫游穿过压缩区自动快进；事件点/人生条预计算 `ev.wy`/`p.wb/p.wd`；标尺按区段选步长（边缘区步长 ×3）；全览/聚焦/快跳目标都过 warpY。视野读数 = 区间数据年跨度（unwarp 差值）。
- **顶栏缩放控件**：log 刻度滑杆（MIN_PPY..MAX_PPY）+ 「视野 N 年」读数，以内容区中心时间为锚调焦；⌘滚轮/捏合/＋－/滑杆/图例跳转全同步。
- **图例即快跳**：8 个朝代色 chip（颜色=数据编码），点击按 `ERA_JUMPS` 跳转并落到存活带；旧的独立快跳按钮排已删（重复）。
- **hover 不变暗**（用户定稿）：只保留金色准线 + 左列年龄；聚焦聚光灯（非聚焦行 0.25/0.3）保留。
