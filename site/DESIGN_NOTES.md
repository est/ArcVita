...
- **滚轮 = 斜向漫游（非欧几里得，2026-09 定稿）**：无修饰滚轮 → 时间速度冲量 `navV`（`exp(-2.8t)` 衰减，上限 1.5 屏年/s），loop 的 `flow` 分支匀速前进 + y 追贴「存活带」——`rowForTime` 二分 t−40 岁 cohort（±60 行内就近落回存活者；空旷段 cohort 线走平 = 纯水平快进时间）。**⌘/ctrl+滚轮 = 缩放**（触控板捏合天然带 ctrlKey，地图惯例）；聚焦/拖拽/tween 中滚轮不漫游；RM 直映无惯性。滚轮向下 = 时间前进，触控板横扫手感待真机调。
...
- **Poincaré 时间压缩**（非线性 年→px 映射）：本轮明确不做（牵动 yearX/xYear 全部调用点、刻度、事件密度、快跳目标）；斜向导航已落地，压缩另立一轮。
